...
export const listings = createTable("listing", {
	...
	images: varchar("images", { length: 2048 })
		.array()
		.default(sql`'{}'::text[]`)
		.notNull(),
	sellerId: varchar("seller_id", { length: 255 }).references(() => users.id), // Company's internal seller (employee/admin)
	evaluatedById: varchar("evaluated_by_id", { length: 255 }).references(
		() => users.id,
	), // Employee who evaluated this listing
	createdAt: timestamp("created_at", { withTimezone: true })
		.defaultNow()
		.notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true })
...
export const tradeInListings = createTable("trade_in_listings", {
	...
	userId: varchar("user_id", { length: 255 })
		...
		.notNull(), // User who submitted the trade-in
	title: varchar("title", { length: 255 }).notNull(),
	...
	condition: pgEnum("TradeInCondition", [
		"Uusi",
		"Kuin uusi",
		"Hyvä",
		"Tyydyttävä",
	...
	evaluatedById: varchar("evaluated_by_id", { length: 255 }).references(
		() => users.id,
	), // Employee who evaluated this trade-in
...
export const purchases = createTable("purchase", {
	id: varchar("id", { length: 255 }).notNull().primaryKey(),
	companyListingId: varchar("company_listing_id", { length: 255 })
...
export const listingsRelations = relations(listings, ({ one, many }) => ({
	...
	evaluatedBy: one(users, {
		fields: [listings.evaluatedById],
		references: [users.id],
		relationName: "evaluatedBy",
	}),
	purchases: many(purchases, { relationName: "listing" }),
}));

export const tradeInListingsRelations = relations(
	tradeInListings,
	({ one }) => ({
		user: one(users, {
			fields: [tradeInListings.userId],
			references: [users.id],
			relationName: "user",
		}),
		evaluatedBy: one(users, {
			fields: [tradeInListings.evaluatedById],
			references: [users.id],
			relationName: "evaluatedBy",
		}),
	}),
...
export const purchasesRelations = relations(purchases, ({ one }) => ({
	companyListing: one(listings, {
		fields: [purchases.companyListingId],
	...
	warranty: one(warranties, {
		fields: [purchases.id],
		references: [warranties.purchaseId],
	}),
}));

export const warrantyRelations = relations(warranties, ({ one }) => ({
	purchase: one(purchases, {
		fields: [warranties.purchaseId],
