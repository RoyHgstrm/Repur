...
const UpdateCompanyListingSchema = z
	.object({
		id: z.string(),
	...
	.refine(
		...
		{
			message: "discountEnd täytyy olla alkamisen jälkeen",
			path: ["discountEnd"],
		},
	);
...
const SearchCompanyListingsSchema = z.object({
	...
	sortBy: z
		.enum(["price-low", "price-high", "newest", "rating"])
		.optional()
		.default("newest"),
	filterCondition: z.string().optional().default("all"),
	perfTier: z
		.enum(["all", "Huippusuoritus", "Erinomainen", "Hyvä", "Perus"])
		.optional()
		.default("all"),
	gpuTier: z
		.enum([
...
export const listingsRouter = createTRPCRouter({
	...
	createCompanyListing: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (user?.role !== "EMPLOYEE" && user?.role !== "ADMIN") {
				throw new TRPCError({
					code: "FORBIDDEN",
					message: "Vain työntekijät ja ylläpitäjät voivat luoda listauksia",
				});
			}
			...
			try {
				const listingsData = await ctx.db.query.listings.findMany({
					where: (onlyFeatured
						? and(
								eq(listings.status, "ACTIVE"),
								eq(listings.isFeatured as any, true as any),
							)
				...
				if (
					/is_featured/i.test(message) ||
					/column\s+"is_featured"/i.test(message)
				) {
	...
	updateCompanyListing: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (user?.role !== "EMPLOYEE" && user?.role !== "ADMIN") {
				throw new TRPCError({
					code: "FORBIDDEN",
					message: "Vain työntekijät ja ylläpitäjät voivat muokata listauksia",
				});
			}
	...
	createTradeInSubmission: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const tradeIn = await ctx.db
				.insert(tradeInListings)
				.values({
					id: nanoid(),
					userId: ctx.userId,
					title: input.title,
	...
	getPendingTradeInListings: protectedProcedure.query(async ({ ctx }) => {
		const user = await ctx.db.query.users.findFirst({
			where: eq(users.id, ctx.userId),
		});
		if (user?.role !== "EMPLOYEE" && user?.role !== "ADMIN") {
			throw new TRPCError({
				code: "FORBIDDEN",
				message:
					"Vain työntekijät ja ylläpitäjät voivat tarkastella trade-in-pyyntöjä",
			});
		}
	...
	getTradeInListings: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const rows = await ctx.db.query.tradeInListings.findMany({
				...(input?.status
					? { where: eq(tradeInListings.status, input.status) }
					: ({} as any)),
				with: { user: true },
			});
			return rows;
	...
	evaluateTradeIn: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const updatedTradeIn = await ctx.db
				.update(tradeInListings)
				.set({
	...
	getUserCompanyListings: protectedProcedure.query(async ({ ctx }) => {
		...
		const listingsWithResolvedImages = await Promise.all(
			userListingsData.map(async (listing) => ({
				...listing,
	...
	getAllCompanyListings: protectedProcedure.query(async ({ ctx }) => {
		...
		const staff = await ctx.db.query.users.findFirst({
			where: eq(users.id, ctx.userId),
		});
		if (staff?.role !== "EMPLOYEE" && staff?.role !== "ADMIN") {
			throw new TRPCError({
				code: "FORBIDDEN",
				message: "Vain henkilöstö voi nähdä kaikki listaukset",
			});
		}
	...
	markTradeInContacted: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (user?.role !== "EMPLOYEE" && user?.role !== "ADMIN") {
				throw new TRPCError({
					code: "FORBIDDEN",
					message: "Vain työntekijät voivat päivittää trade-in-tilanteita",
				});
			}
