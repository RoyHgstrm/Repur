...
export default clerkMiddleware(async (authCtx, req: NextRequest) => {
  ...
  console.log('Middleware: Supabase Client Initialized with URL:', process.env.NEXT_PUBLIC_SUPABASE_URL);
  ...
  if (userId) {
    try {
      ...
      console.error('Middleware: Error setting Supabase session from Clerk token:', error);
    }
  }
  ...
  if (sessionError) {
    console.error('Middleware: Supabase Session Error after sync:', sessionError);
  }
...
export const config = {
  matcher: ['/((?!.*\\..*|_next).*)-match', '/', '/(api|trpc)(.*)', '/(admin)(.*)']
};
