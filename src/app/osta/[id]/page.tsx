"use client";
// Client component for listing details
...
export default function ListingDetailPage() {
	...
	const favState = trpc.favorites.isFavorited.useQuery(
		{ listingId: id },
		{ enabled: !!id && Boolean(isSignedIn) },
	);
	...
	useEffect(() => {
		...
		const incrementViewCount = async () => {
			if (!id || !isAuthLoaded || viewIncrementedRef.current || !mounted)
				return;
	...
	const [estimatedGameFps, setEstimatedGameFps] = useState(() =>
		getEstimatedFps(3, null, null),
	);
	...
	const performanceScore = listing
		...
		: 0;
	const ratingForStars = Math.max(
		1,
		Math.min(5, Math.round(performanceScore / 20)),
	);
	...
	useEffect(() => {
		setEstimatedGameFps(
			getEstimatedFps(ratingForStars, gpuForCalc, cpuForCalc),
		);
	...
	function getGpuTierMultiplier(gpu: string | null): number {
		...
		if (
			/rtx\s*3090|rtx\s*3080|rtx\s*3070|rx\s*69\d{2}|rx\s*68\d{2}|rx\s*67\d{2}/.test(
				g,
			)
		)
			return 1.2;
	...
	function getEstimatedFps(
		...
		const estimatedFpsMap = new Map<
			string,
			Map<string, { quality: string; fps: number }[]>
		>();

		allFpsData.forEach(({ game, resolution, quality, multiplier }) => {
			const perfBase =
				FPS_DATA.BASE_FPS + (rating - 3) * FPS_DATA.RATING_ADJUST;
			const tuning = Math.min(
				1.5,
				Math.max(0.75, getGpuTierMultiplier(gpu) * getCpuTierMultiplier(cpu)),
			);
			...
			if (!estimatedFpsMap.has(game)) {
				estimatedFpsMap.set(
					game,
					new Map<string, { quality: string; fps: number }[]>(),
				);
			}
				...
				<div className="min-h-screen bg-gradient-to-br from-[var(--color-surface-1)] via-[var(--color-surface-2)] to-[var(--color-surface-1)] flex items-center justify-center">
					<div className="text-center space-y-4">
						...
						<p className="text-secondary">
							Pyydettyä kohdetta ei ole tai se on poistettu.
						</p>
	...
	const specs = [
		...
		{
			label: "Emolevy",
			value: listing.motherboard,
			Icon: Server,
			color: "text-[var(--color-secondary)]",
		},
		{
			label: "Virtalähde",
			value: listing.powerSupply,
			Icon: Power,
			color: "text-[var(--color-warning)]",
		},
		{
			label: "Kotelo",
			value: listing.caseModel,
			Icon: Package,
						...
						<div className="flex items-center gap-2">
							<Button
								...
								onClick={() =>
									favToggle.mutate(
										{ listingId: id },
										{ onSuccess: () => favState.refetch() },
									)
								}
								className={cn(
									"p-2 rounded-full",
									favState.data?.favorited && "text-[var(--color-success)]",
								)}
							>
						...
						<Card className="overflow-hidden bg-gradient-to-br from-surface-2 to-surface-1 border-[var(--color-border-light)] shadow-lg hover:shadow-xl transition-all duration-500">
							<CardContent className="p-0">
								<div className="space-y-0">
									...
									<div
										className="relative aspect-video overflow-hidden group cursor-pointer rounded-lg"
											...
											<div className="w-full h-full min-h-[180px] flex items-center justify-center bg-gradient-to-br from-[var(--color-surface-3)] to-[var(--color-surface-2)] rounded-lg">
												<div className="flex flex-col items-center justify-center text-[var(--color-neutral)]/60">
													<div className="relative">
														...
														<div
															className="absolute inset-0 bg-gradient-to-r from-[var(--color-primary)]/20 to-[var(--color-accent)]/20 blur-xl scale-150"
															aria-hidden
														/>
													</div>
						...
						<Card className="bg-surface-2 border-[var(--color-border-light)]">
							<CardHeader className="pb-3">
								...
								<CardDescription className="text-secondary mt-2">
									Suuntaa-antavat ruudunpäivitysnopeudet eri resoluutioilla ja
									asetuksilla.
								</CardDescription>
							</CardHeader>
								...
								{!selectedGame && (
									<div className="flex flex-col items-center justify-center text-center py-8 bg-surface-1 rounded-lg border border-[var(--color-border)]">
										...
										<p className="text-sm text-secondary">
											Valitse peli nähdäksesi arvioidun FPS:n eri asetuksilla
										</p>
									</div>
								)}
								{selectedGame &&
									(() => {
										const gameData = estimatedGameFps.find(
											(g) => g.game === selectedGame,
										);
											...
											<div className="bg-gradient-to-b from-surface-1 to-surface-2 rounded-xl border border-[var(--color-border)] p-4">
												...
												<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
													{gameData.resolutions.map((resData, resIdx) => (
														<div
															key={resIdx}
															className="rounded-lg bg-surface-1 border border-[var(--color-border)] p-3"
														>
					...
					<div className="order-1 xl:order-2 space-y-6 md:max-w-[640px] md:mx-auto xl:max-w-none xl:mx-0 w-full">
						...
						<Card className="w-full xl:sticky xl:top-24 md:rounded-lg bg-gradient-to-br from-surface-2 to-surface-3 border-[var(--color-border-light)] shadow-xl">
							<CardHeader className="pb-4">
								...
								{/* Performance Rating */}
								<div
									className="flex flex-col gap-2 xs:gap-3 p-3 bg-surface-1 rounded-lg
							...
							<CardContent className="space-y-6">
								...
								{(() => {
									...
									const discountAmountNum =
										"discountAmount" in listing &&
										(listing as any).discountAmount != null
											? Number((listing as any).discountAmount)
											: 0;
									const now = new Date();
									const hasWindow =
										"discountStart" in listing &&
										"discountEnd" in listing &&
										(listing as any).discountStart &&
										(listing as any).discountEnd
											? now >=
													new Date(String((listing as any).discountStart)) &&
												now <= new Date(String((listing as any).discountEnd))
											: false;
									const finalPrice =
											...
											{hasWindow && discountAmountNum > 0 ? (
												<div className="space-y-2">
													...
													<div className="text-xs text-tertiary">
														Alennus voimassa{" "}
														{new Date(
															String((listing as any).discountEnd),
														...
														onClick={(e) => {
															...
															try {
																...
																const signedIn =
																	...
																	(window as any)?.__clerk?.user?.id != null;
																if (!signedIn) {
																	e.preventDefault();
														...
														{(() => {
															...
															const hasWindow =
																...
																(listing as any).discountEnd
																	? now >=
																			new Date(
																				String((listing as any).discountStart),
																			) &&
																		now <=
																			new Date(
																				String((listing as any).discountEnd),
															...
															const finalPrice =
																hasWindow && discountAmountNum > 0
																	? Math.max(
																			0,
																			basePriceNum - discountAmountNum,
																		)
																	: basePriceNum;
													...
													const hasWindow =
														...
														(listing as any).discountEnd
															? now >=
																...
																now <=
																	new Date(String((listing as any).discountEnd))
															: false;
								...
								<div className="space-y-3 pt-4 border-t border-[var(--color-border)]">
									...
									<div className="space-y-2">
										{[
											...
											{
												icon: Truck,
												text: "Ilmainen toimitus",
												color: "text-[var(--color-primary)]",
											},
											{
												icon: CheckCircle,
												text: "Testattu ja kunnostettu",
												color: "text-[var(--color-secondary)]",
											},
											{
												icon: User,
												text: "Asiantunteva tuki",
												color: "text-[var(--color-accent)]",
											},
										].map((item, idx) => (
											<div
					...
					<Card className="bg-surface-2 border-[var(--color-border-light)]">
						...
						<CardContent className="p-4 sm:p-6 md:p-8 space-y-4">
							<CollapsibleComponent
								title="Toimitus"
								icon={
									<Truck className="h-5 w-5 min-w-[1.25rem] min-h-[1.25rem]" />
								}
							>
								<p className="text-sm sm:text-base text-[var(--color-neutral)]/80 leading-relaxed">
									Verkkokaupassamme asiointi on helppoa ja vaivatonta aina
									tilaamisesta toimitukseen asti. Kaikki tilaukset lähetetään
									aina viimeistään seuraavana arkipäivänä tilauksen tekemisestä,
									joten voit odottaa pakettisi saapuvan nopeasti – tyypillisesti
									2-4 arkipäivän kuluessa. Lisäksi tarjoamme kaikille
									tilauksille ilmaisen toimituksen, joten voit tehdä ostoksia
									ilman ylimääräisiä kuluja.
									<br />
									<br />
									Pidämme sinut ajan tasalla tilauksesi etenemisestä ja
									ilmoitamme, kun pakettisi on matkalla.
								</p>
							</CollapsibleComponent>
							...
							<CollapsibleComponent
								...
								icon={
									<ShieldCheck className="h-5 w-5 min-w-[1.25rem] min-h-[1.25rem]" />
								}
							>
								<h4 className="font-semibold text-sm sm:text-base text-[var(--color-neutral)] mb-2">
									Palautusoikeus
								</h4>
								<p className="text-sm sm:text-base text-[var(--color-neutral)]/80 leading-relaxed mb-4">
									Haluamme, että olet täysin tyytyväinen ostoksiisi. Siksi
									tarjoamme kaikille tuotteille 14 päivän palautusoikeuden. Jos
									jostain syystä et ole tyytyväinen ostokseesi, voit palauttaa
									sen alkuperäisessä kunnossa ja pakkauksessa 14 päivän kuluessa
									tilauksen vastaanottamisesta. Palautus on maksuton.{" "}
									<Link
										href="/takuu"
										className="text-[var(--color-primary)] hover:underline"
									>
										Voit lukea lisää palautusehdoistamme.
									</Link>
								</p>
								<h4 className="font-semibold text-sm sm:text-base text-[var(--color-neutral)] mb-2">
									Vuoden takuu
								...
								<p className="text-sm sm:text-base text-[var(--color-neutral)]/80 leading-relaxed">
									Kaikilla tuotteillamme on vuoden takuu, joka kattaa tekniset
									viat. Takuu alkaa ostopäivästä ja se kattaa valmistus- ja
									materiaalivirheet, jotka vaikuttavat tuotteen normaaliin
									käyttöön. Mikäli tuotteessasi ilmenee takuuaikana teknisiä
									vikoja, korjaamme tai vaihdamme tuotteen maksutta.{" "}
									<Link
										href="/takuu"
										className="text-[var(--color-primary)] hover:underline"
									>
										Voit lukea takuusta lisää takuuehdoistamme.
									</Link>
									<br />
									<br />
									Jos sinulla on kysyttävää takuusta tai palautusprosessista,
									ota rohkeasti yhteyttä asiakaspalveluumme. Olemme täällä
									auttaaksemme!
								</p>
							</CollapsibleComponent>
							...
							<CollapsibleComponent
								...
								icon={
									<Zap className="h-5 w-5 min-w-[1.25rem] min-h-[1.25rem]" />
								}
							>
								<p className="text-sm sm:text-base text-[var(--color-neutral)]/80 leading-relaxed mb-3">
									Repur RESPAWN™ Huoltosopimus on avain huolettomaan
									pelikokemukseen ja täydelliseen mielenrauhaan. Kun ostat
									koneesi Repurlta, saat rajoitetun ajan täysin maksutta
									kattavan palvelupaketin, joka sisältää:
								</p>
								...
								<Link
									href="/tuki"
									className="text-[var(--color-primary)] hover:underline text-sm sm:text-base mt-3 inline-block"
								>
