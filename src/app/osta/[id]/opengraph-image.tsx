...
export default async function Image({ params }: { params: { id: string } }) {
  try {
    ...
    const discountAmountNum = listing.discountAmount ? Number(listing.discountAmount) : 0
    const now = new Date()
    const hasDiscount = listing.discountStart && listing.discountEnd
      ? now >= new Date(listing.discountStart) && now <= new Date(listing.discountEnd)
      : false
    const finalPrice = hasDiscount && discountAmountNum > 0
        ...
        fonts: [
          {
            name: 'Inter',
            data: await fetch(
              new URL('https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuI6fAZJhiI2B.woff2', 'https://fonts.gstatic.com')
          ...
          },
          {
            name: 'Inter',
            data: await fetch(
              new URL('https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuFuYAZJhiI2B.woff2', 'https://fonts.gstatic.com')
            ).then((res) => res.arrayBuffer()),
            weight: 700,
            style: 'normal',
          },
