		...
		for (const file of files) {
			if (file instanceof Blob) {
				if (file.size > 5 * 1024 * 1024) {
					return NextResponse.json(
						{ error: `Kuva ${file.name} on liian suuri (max 5 Mt)` },
						{ status: 400 },
					);
				}
				...
				if (!["image/jpeg", "image/png"].includes(file.type)) {
					return NextResponse.json(
						{ error: `Vain JPG/PNG-tiedostot ovat sallittuja: ${file.name}` },
						{ status: 400 },
					);
				}

				const originalName =
					typeof (file as any).name === "string" ? (file as any).name : "image";
