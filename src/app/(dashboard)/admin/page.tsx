...
import {
	...
	getPerformanceTier,
} from "~/lib/performanceScoring"; // HOW: Import the new, comprehensive performance scoring utility.
...
const CompanyListingSchema = z.object({
	...
	description: z
		.string()
		.min(10, "Kuvaus on liian lyhyt")
		.max(2048, "Kuvaus on liian pitkä"),
	cpu: z.string(),
	gpu: z.string(),
	ram: z.string(),
	storage: z.string(),
...
function AdminStats() {
	const { data: activeListings } =
		api.listings.getActiveCompanyListings.useQuery();
	const { data: pendingTradeIns } =
		api.listings.getPendingTradeInListings.useQuery();
	...
	return (
		<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
			{stats.map((s, i) => (
				<Card
					key={i}
					className="bg-[var(--color-surface-2)] border-[var(--color-border)]"
				>
					<CardContent className="p-4 flex items-center justify-between">
						<div>
							...
							<div className="text-2xl-fluid font-bold text-[var(--color-text-primary)]">
								{s.value}
							</div>
						</div>
		...
		<Card className="bg-[var(--color-surface-2)] border-[var(--color-border)] text-[var(--color-neutral)]">
			<CardHeader>
				<CardTitle className="text-2xl-fluid font-semibold flex items-center gap-2">
					<TrendingUp className="w-5 h-5" /> Myyntianalytiikka
				</CardTitle>
			</CardHeader>
					...
					<Card className="bg-[var(--color-surface-3)] border-[var(--color-border)]">
						<CardContent className="p-4">
							...
							<div className="text-2xl-fluid font-bold">
								{stats?.last7.count ?? 0} kpl
							</div>
							<div className="text-sm text-[var(--color-text-secondary)]">
								{stats?.last7.revenue ?? 0} €
							</div>
						</CardContent>
					</Card>
					<Card className="bg-[var(--color-surface-3)] border-[var(--color-border)]">
						<CardContent className="p-4">
							...
							<div className="text-2xl-fluid font-bold">
								{stats?.last30.count ?? 0} kpl
							</div>
							<div className="text-sm text-[var(--color-text-secondary)]">
								{stats?.last30.revenue ?? 0} €
							</div>
						</CardContent>
					</Card>
				</div>
...
function PurchasesTable() {
	...
	const { data, isLoading, refetch } = trpc.purchases.getAll.useQuery(
		{ q },
		{ refetchOnWindowFocus: false },
	);
	const refundMutation = trpc.purchases.refund.useMutation({
		onSuccess: () => {
			toast({
				title: "Palautus aloitettu",
				description: "Palautus luotu Stripeen.",
				variant: "success",
			});
			void refetch();
		},
	...
	return (
		<Card className="bg-[var(--color-surface-2)] border-[var(--color-border)]">
			<CardHeader>
				<CardTitle className="text-2xl-fluid font-semibold flex items-center gap-2">
					<Receipt className="w-5 h-5" /> Ostot
				</CardTitle>
			</CardHeader>
			<CardContent className="space-y-3">
				<div className="flex gap-2">
					<Input
						placeholder="Hae (otsikko, email, ID)"
						value={q}
										...
										<td className="py-2 pr-3">
											<div className="flex gap-2">
												<Button
													...
													onClick={() =>
														window.open(
															`https://dashboard.stripe.com/test/payments?search=${p.id}`,
															"_blank",
														)
													}
												>
												...
												<Button
													size="sm"
													variant="destructive"
													disabled={refundMutation.status === "pending"}
													onClick={() =>
														refundMutation.mutate({ purchaseId: p.id })
													}
												>
													Palauta
												</Button>
...
function WarrantiesTable() {
	const [q, setQ] = useReactState("");
	const { data, isLoading, refetch } = trpc.warranties.getAll.useQuery(
		{ q },
		{ refetchOnWindowFocus: false },
	);
	const update = trpc.warranties.update.useMutation({
		onSuccess: () => {
			toast({ title: "Takuuta päivitetty", variant: "success" });
			void refetch();
		},
		onError: (e) =>
			toast({ title: "Virhe", description: e.message, variant: "destructive" }),
	...
	return (
		<Card className="bg-[var(--color-surface-2)] border-[var(--color-border)]">
			<CardHeader>
				<CardTitle className="text-2xl-fluid font-semibold flex items-center gap-2">
					<ShieldCheck className="w-5 h-5" /> Takuut
				</CardTitle>
			</CardHeader>
			<CardContent className="space-y-3">
				<div className="flex gap-2">
								...
								{(data ?? []).map((w) => {
									...
									return (
										...
										>
											...
											<td className="py-2 pr-3">
												{w.purchase?.buyer?.email ?? "—"}
											</td>
											<td className="py-2 pr-3">
												{w.purchase?.companyListing?.title ?? "—"}
											</td>
											<td className="py-2 pr-3">
												{start.toLocaleDateString("fi-FI")} –{" "}
												{end.toLocaleDateString("fi-FI")}
											</td>
											<td className="py-2 pr-3">
												{w.status}
												{!valid ? " (päättynyt)" : ""}
											...
											<td className="py-2 pr-3">
												<div className="flex gap-2">
													<Button
														...
														variant="outline"
														onClick={() =>
															update.mutate({ id: w.id, status: "ACTIVE" })
														}
													>
														Aktivoi
													</Button>
													<Button
														...
														variant="outline"
														onClick={() =>
															update.mutate({ id: w.id, status: "CLAIMED" })
														}
													>
														Merkitse käytetyksi
													</Button>
													<Button
														...
														variant="outline"
														onClick={() =>
															update.mutate({ id: w.id, status: "EXPIRED" })
														}
													>
														Päättynyt
													</Button>
...
function CompanyListingForm() {
	...
	useEffect(() => {
		...
		const tierKey = tier.level.toUpperCase() as
			| "PLATINUM"
			| "GOLD"
			| "SILVER"
			| "BRONZE";
		...
		const cleanCpu = formData.cpu
			? formData.cpu
					.toUpperCase()
					.replace(/(INTEL\\s*CORE\\s*|AMD\\s*RYZEN\\s*)/g, "")
					.trim()
			: "Ei tietoa";
		const cleanGpu = formData.gpu
			? formData.gpu
					.toUpperCase()
					.replace(
						/(NVIDIA\\s*GEFORCE\\s*RTX\\s*|AMD\\s*RADEON\\s*RX\\s*)/g,
						"",
					)
					.trim()
			: "Ei tietoa";
		...
		if (tierRegex.test(currentTitle)) {
			...
			const finalCpuForTitle =
				existingCpuPart !== "Ei tietoa" && existingCpuPart !== ""
					? existingCpuPart
					: cleanCpu;
			const finalGpuForTitle =
				existingGpuPart !== "Ei tietoa" && existingGpuPart !== ""
					? existingGpuPart
					: cleanGpu;
		...
		const isAutoDesc = Object.values(AUTO_DESCRIPTIONS).some(
			(d) =>
				currentDesc.startsWith(d.substring(0, 50)) &&
				currentDesc.length >= d.length * 0.8 &&
				currentDesc.length <= d.length * 1.2,
		); // Check start of string and length
		const nextDesc = AUTO_DESCRIPTIONS[tierKey];

		setFormData((prev) => ({
			...prev,
			...(nextTitle !== prev.title ? { title: nextTitle } : {}), // Only update if changed
			...(currentDesc.length === 0 || isAutoDesc
	...
	const AUTO_DESCRIPTIONS: Record<
...
Kuten muissakin malleissamme, mukana on 14 päivän palautusoikeus ja 12 kuukauden takuu.`,
	...
	const createCompanyListingMutation =
		api.listings.createCompanyListing.useMutation({
			onSuccess: () => {
				toast({
					title: "Listaus luotu",
	...
	const handleInputChange = (
		field: keyof CompanyListingFormData,
		value: string,
	) => {
		setFormData((prev) => ({
			...prev,
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="space-y-2">
						<Label
							...
							className="text-[var(--color-neutral)] font-semibold"
						>
							Tuotekuvat (max 10)
						</Label>
						<Input
							...
							onChange={async (e) => {
								...
								try {
									...
									setFormData((p) => ({
										...p,
										images: [...(p.images ?? []), ...data.imageUrls],
									}));
									toast({
										title: "Kuvat ladattu",
										description: `${files.length} kuva(a) lisätty`,
										variant: "success",
									});
								} catch (err: any) {
									toast({
										title: "Virhe",
										description: err?.message ?? "Kuvien lataus epäonnistui",
										variant: "destructive",
									});
								} finally {
									if (fileInputRef.current) fileInputRef.current.value = "";
										...
										>
											...
											<div className="absolute inset-x-0 bottom-0 flex gap-1 p-1 bg-[var(--color-surface-2)]/85">
												...
												<Button
													...
													className="h-7 px-2 text-xs"
													onClick={() => {
														if (idx === (formData.images?.length ?? 1) - 1)
															return;
														setFormData((p) => {
															const next = [...(p.images ?? [])];
															[next[idx + 1], next[idx]] = [
					...
					<div>
						...
						>
							...
							<SelectContent className="bg-[var(--color-surface-3)] border-[var(--color-border)] text-[var(--color-neutral)]">
								...
								<SelectItem value="Kuin uusi">
									Kuin uusi (erinomainen)
								</SelectItem>
								<SelectItem value="Hyvä">
									Hyvä (pieniä käytön jälkiä)
								</SelectItem>
								<SelectItem value="Tyydyttävä">
									Tyydyttävä (selviä naarmuja)
								</SelectItem>
							</SelectContent>
						</Select>
					</div>
					...
					<Button
						type="submit"
						className="w-full bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/90 text-white py-2 px-4 rounded-md transition-colors duration-200"
					>
						Luo Listaus
					</Button>
				</form>
...
function TradeInListingsTable() {
	const [statusFilter, setStatusFilter] = useReactState<
		"ALL" | "PENDING" | "CONTACTED" | "ACCEPTED" | "REJECTED" | "COMPLETED"
	>("PENDING");
	const [search, setSearch] = useReactState("");
	const {
		data: tradeInListings,
		isLoading,
		refetch,
	} = api.listings.getTradeInListings.useQuery(
	...
	const evaluateMutation = api.listings.evaluateTradeIn.useMutation({
		onSuccess: () => {
			toast({
				title: "Päivitetty",
				description: "Trade-in pyyntö päivitetty.",
				variant: "success",
			});
			void refetch();
		},
	...
	return (
		<Card className="bg-[var(--color-surface-2)] border-[var(--color-border)] text-[var(--color-neutral)]">
			...
			<CardContent>
				...
				{isLoading ? (
					<p className="text-center text-[var(--color-neutral)]/80">
						Ladataan trade-in pyyntöjä...
					</p>
				) : (tradeInListings ?? []).length === 0 ? (
					<p className="text-center text-[var(--color-neutral)]/80">
						Ei odottavia trade-in pyyntöjä.
					</p>
				) : (
					<div className="space-y-4">
						{(tradeInListings ?? [])
							...
							.map((listing: TradeInListing) => (
								<Card
									key={listing.id}
									className="bg-[var(--color-surface-3)] border-[var(--color-border)] p-4"
								>
									<h3 className="text-lg-fluid font-semibold text-[var(--color-neutral)]">
										{listing.title}
									</h3>
									<p className="text-[var(--color-neutral)]/80 text-sm">
										Käyttäjä: {listing.user?.name ?? "Tuntematon"}
									</p>
									<p className="text-[var(--color-neutral)]/80 text-sm">
										Sähköposti: {listing.contactEmail}
									</p>
									{/* Puhelinnumero poistettu UI:sta */}
									<p className="text-[var(--color-neutral)]/80 text-sm">
										Prosessori: {listing.cpu}
									</p>
									<p className="text-[var(--color-neutral)]/80 text-sm">
										Näytönohjain: {listing.gpu}
									</p>
									<p className="text-[var(--color-neutral)]/80 text-sm">
										RAM: {listing.ram}
									</p>
									<p className="text-[var(--color-neutral)]/80 text-sm">
										Tallennustila: {listing.storage}
									</p>
									<p className="text-[var(--color-neutral)]/80 text-sm">
...
function CompanyListingsManage() {
	...
	const { data: allListings } = api.listings.getAllCompanyListings.useQuery(
		undefined,
	...
	const {
		...
		isLoading,
		refetch,
	} = api.listings.getUserCompanyListings.useQuery(undefined, {
		refetchOnWindowFocus: false,
	});
	...
	const [sortBy, setSortBy] = useReactState<
		"RECENT" | "PRICE_ASC" | "PRICE_DESC" | "TITLE"
	>("RECENT");
	...
	const [bulkPresetDays, setBulkPresetDays] = useReactState<
		"1" | "3" | "7" | "14" | "30"
	>("7");

	const sourceRaw = useMemo(
		() => (onlyMine ? (myListings ?? []) : (allListings ?? myListings ?? [])),
		[onlyMine, myListings, allListings],
	);
	...
	const bulkUpdateStatus = async (
		newStatus: "ACTIVE" | "ARCHIVED" | "SOLD",
	) => {
		...
		try {
			...
			toast({
				title: "Päivitetty",
				description: "Valitut listaukset päivitetty.",
				variant: "success",
			});
		} catch (e: any) {
			toast({
				title: "Virhe",
				description: e?.message ?? "Päivitys epäonnistui",
				variant: "destructive",
			});
		}
	};
	...
	const applyBulkDiscount = async () => {
		...
		if (!Number.isFinite(amount) || amount <= 0) {
			toast({
				title: "Virheellinen alennus",
				description: "Syötä positiivinen euromäärä.",
				variant: "destructive",
			});
			return;
		}
		const now = new Date();
		const end = new Date(
			now.getTime() + Number(bulkPresetDays) * 24 * 60 * 60 * 1000,
		);
		...
		try {
			...
			toast({
				title: "Alennus asetettu",
				description: `Asetettiin ${amount} € alennus ${bulkPresetDays} päivän ajaksi (${selectedIds.size} kpl).`,
				variant: "success",
			});
			setSelectedIds(new Set());
			void refetch();
			...
			toast({
				title: "Virhe",
				description: e?.message ?? "Alennusten asettaminen epäonnistui",
				variant: "destructive",
			});
		}
	};
			...
			<CardContent className="space-y-3">
				...
				<div className="flex flex-wrap gap-3 items-center justify-between">
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						/>
						<Select
							value={bulkPresetDays}
							onValueChange={(v) => setBulkPresetDays(v as any)}
						>
					...
					<div className="overflow-x-auto">
						<table className="w-full text-sm overflow-x-auto block">
							<thead>
								<tr className="text-left text-[var(--color-text-tertiary)] border-b border-[var(--color-border)]/50 flex flex-wrap sm:table-row">
									<th className="py-2 pr-3 w-full sm:w-auto sm:table-cell">
										<input
											type="checkbox"
											...
											checked={
												paged.length > 0 &&
												paged.every((x) => selectedIds.has(x.id))
											}
										/>
									</th>
									<th className="py-2 pr-3 w-full sm:w-auto sm:table-cell">
									...
									<th className="py-2 pr-3 w-full sm:w-40 sm:table-cell">
										Toiminnot
									</th>
								</tr>
							</thead>
							<tbody className="block sm:table-row-group">
								{paged.map((l) => (
									<tr
										key={l.id}
										className="border-b border-[var(--color-border)]/50 flex flex-col sm:table-row"
									>
										<td className="py-1 px-2 align-middle sm:table-cell">
											<input
												type="checkbox"
												aria-label={`Valitse ${l.title ?? l.id}`}
						...
						<div className="flex flex-wrap items-center justify-between mt-2 text-xs">
							<div className="text-[var(--color-text-tertiary)]">
								Sivu {pageSafe}/{totalPages} • {filtered.length} listaus(ta)
							</div>
							<div className="flex gap-2">
								<Button
									size="sm"
...
function ManageRow({ listing, onSave }: ManageRowProps) {
	...
	const [discountAmount, setDiscountAmount] = useReactState<string>(
	...
	return (
		<>
			...
			<td className="py-1 px-2 align-middle">
				<FeatureToggle
					id={listing.id}
					featured={(listing as any).isFeatured === true}
					onChanged={() => void 0}
				/>
			</td>
