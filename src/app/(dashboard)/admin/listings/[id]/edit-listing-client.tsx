...
type ListingWithResolvedImages = RouterOutputs["listings"]["getCompanyListingById"] & {
...
export default function EditListingPage({ initialListing }: { initialListing: ListingWithResolvedImages }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of selectedFiles) {
      if (file.size > 5 * 1024 * 1024) {
        toast({ title: 'Virhe', description: `Kuva ${file.name} on liian suuri (max 5 Mt).`, variant: 'destructive' });
        continue;
      }
      if (!['image/jpeg', 'image/png'].includes(file.type)) {
        toast({ title: 'Virhe', description: `Kuva ${file.name} on väärää tyyppiä. Vain JPG/PNG sallittu.`, variant: 'destructive' });
        continue;
      }
    ...
    for (const file of validFiles) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || `Kuvan ${file.name} lataus epäonnistui.`);
        }
        const result = await response.json() as { imageUrls: string[] }; // Expect imageUrls array
        setForm((prev) => ({ ...prev, images: [...prev.images, ...result.imageUrls] })); // Add new image paths
        toast({ title: 'Onnistui', description: `Kuva ${file.name} ladattu.`, variant: 'success' });
      } catch (error: any) {
        toast({ title: 'Virhe', description: error?.message ?? `Kuvan ${file.name} lataus epäonnistui.`, variant: 'destructive' });
      }
    }
  ...
  const submit = () => {
    ...
    const pushIfChanged = <K extends keyof typeof form>(key: K, map?: (v: string) => any) => {
      ...
      const current = (initialListing?.[key as keyof typeof initialListing] as any) ?? "";
      const next = form[key];
                      ...
                      <div key={`existing-${url}-${idx}`} className="relative group rounded-lg overflow-hidden border border-[var(--color-border)] bg-[var(--color-surface-3)]">
                        ...
                        <div className="absolute inset-x-0 bottom-0 flex gap-1 p-1 bg-[var(--color-surface-2)]/85">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setForm((p) => ({ ...p, images: (p.images ?? []).filter((_img: string, i: number) => i !== idx) }));
                            }}
