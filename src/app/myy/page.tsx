...
const calculatePartValue = (part: Part, type: "CPU" | "GPU"): number => {
	...
	const depreciatedValue = basePrice * (1 - depreciationRate) ** age;
	return depreciatedValue;
};

const calculateRamValue = (data: FormData): number => {
	const size = data.ramSize
		? parseInt(data.ramSize, 10)
		: PRICING_CONFIG.RAM.DEFAULT_GB;
	const type =
		data.ramType && data.ramType !== "en_tieda_ram_type"
			? data.ramType
			: "en_tieda_ram_type";
	...
	return (
		size *
		PRICING_CONFIG.RAM.BASE_VALUE_PER_GB *
		typeMultiplier *
		speedMultiplier
	);
};
...
const calculateStorageValue = (data: FormData): number => {
	const size = data.storageSize
		? parseInt(data.storageSize, 10)
		: PRICING_CONFIG.STORAGE.DEFAULT_GB;
	const type =
		data.storageType && data.storageType !== "en_tieda_storage_type"
			? data.storageType
			: "en_tieda_storage_type";
	const typeKey = type as keyof typeof PRICING_CONFIG.STORAGE.BASE_VALUE_PER_GB;
	const multiplier =
		PRICING_CONFIG.STORAGE.BASE_VALUE_PER_GB[typeKey] ||
		PRICING_CONFIG.STORAGE.BASE_VALUE_PER_GB.en_tieda_storage_type;
	return size * multiplier;
};

const calculatePsuValue = (data: FormData): number => {
	const wattage = data.psuWattage
		? parseInt(data.psuWattage, 10)
		: PRICING_CONFIG.PSU.DEFAULT_WATTAGE;
	const efficiency =
		data.psuEfficiency && data.psuEfficiency !== "en_tieda_psu_efficiency"
			? data.psuEfficiency
			: "en_tieda_psu_efficiency";
	const efficiencyKey =
		efficiency as keyof typeof PRICING_CONFIG.PSU.EFFICIENCY_MULTIPLIER;
	const multiplier =
		PRICING_CONFIG.PSU.EFFICIENCY_MULTIPLIER[efficiencyKey] ||
		PRICING_CONFIG.PSU.EFFICIENCY_MULTIPLIER.en_tieda_psu_efficiency;
...
const calculatePrice = (data: FormData): number | null => {
	...
	const mbQualityKey = (data.motherboardQuality ??
		"en_tieda_mb_quality") as keyof typeof PRICING_CONFIG.MOTHERBOARD.QUALITY_MULTIPLIER;
	const mbMultiplier =
		PRICING_CONFIG.MOTHERBOARD.QUALITY_MULTIPLIER[mbQualityKey] ?? 1.0;

	const totalMarketValue =
		(cpuValue + gpuValue + ramValue + storageValue + psuValue) * mbMultiplier;
	const conditionKey = data.condition;
	const conditionMultiplier =
		PRICING_CONFIG.CONDITION_MULTIPLIERS[conditionKey] ||
		PRICING_CONFIG.CONDITION_MULTIPLIERS["En tiedä"];
...
const FormStep = ({
	...
	>
		<div className="text-center mb-4 sm:mb-6">
			<h2 className="text-2xl-fluid sm:text-3xl-fluid font-bold text-[var(--color-neutral)] mb-2">
				{title}
			</h2>
...
export default function MyyPage() {
	...
	const processedCpuOptions: ComboboxOption[] = useMemo(() => {
		return processAndGroupParts(
			...
			"name",
			(cpu) =>
				cpu.name.includes("Ryzen") ? "AMD Processors" : "Intel Processors",
	...
	const RAM_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_ram_size",
				value: "en_tieda_ram_size",
				label: "En tiedä",
			},
		],
		[],
	...
	const RAM_TYPE_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_ram_type",
				value: "en_tieda_ram_type",
				label: "En tiedä",
			},
		],
		[],
	...
	const RAM_SPEED_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_ram_speed",
				value: "en_tieda_ram_speed",
				label: "En tiedä",
			},
		],
		[],
	...
	const STORAGE_TYPE_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_storage_type",
				value: "en_tieda_storage_type",
				label: "En tiedä",
			},
		],
		[],
	...
	const STORAGE_SIZE_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_storage_size",
				value: "en_tieda_storage_size",
				label: "En tiedä",
			},
		],
		[],
	...
	const PSU_WATTAGE_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_psu_wattage",
				value: "en_tieda_psu_wattage",
				label: "En tiedä",
			},
		],
		[],
	...
	const PSU_EFFICIENCY_OPTIONS = useMemo(
		() => [
			...
			{
				id: "en_tieda_psu_efficiency",
				value: "en_tieda_psu_efficiency",
				label: "En tiedä / Muu",
			},
		],
		[],
		...
		() => [
			{
				id: "brand_oem",
				value: "brand_oem",
				label: "Brändiemolevy (HP, Dell, Lenovo...)",
			},
			{
				id: "standard",
				value: "standard",
				label: "Perusemolevy (ATX/mATX ilman erikoisominaisuuksia)",
			},
			{
				id: "premium_custom",
				value: "premium_custom",
				label: "Laadukas kustomoitu emolevy (esim. VRM/PCIe5/Wi‑Fi 6E)",
			},
			{
				id: "en_tieda_mb_quality",
				value: "en_tieda_mb_quality",
				label: "En tiedä",
			},
		],
		[],
	...
	const CONDITION_OPTIONS = useMemo(
		() => [
			{ id: "uusi", value: "Uusi", label: "Uusi (sinetöity)" },
			{
				id: "erinomainen",
				value: "Kuin uusi",
				label: "Erinomainen (kuin uusi)",
			},
			{
				id: "hyväkuntoinen",
				value: "Hyvä",
				label: "Hyvä (pieniä käytön jälkiä)",
			},
			{
				id: "tyydyttävä",
				value: "Tyydyttävä",
				label: "Tyydyttävä (selviä naarmuja)",
			},
			{
				id: "huonokuntoinen",
				value: "Huono",
				label: "Huono (kolhuja, vikoja)",
			},
	...
	const createTradeInSubmissionMutation =
		api.listings.createTradeInSubmission.useMutation({
			onSuccess: () => {
				toast({
					title: "Pyyntö lähetetty",
					description: "Olemme sinuun yhteydessä sähköpostitse.",
					variant: "success",
				});
		...
		try {
			const submissionData = {
				...
				description: [
					...
					formData.motherboardQuality
						? `Emolevy: ${formData.motherboardQuality}`
						: undefined,
				]
					.filter(Boolean)
					.join("\n"),
			...
			<div className="relative overflow-hidden">
				...
				<div className="container-responsive py-section relative">
					<div className="text-center space-y-6 max-w-4xl mx-auto">
						...
						>
							Anna vanhalle pelikoneellesi uusi elämä. Saat reilun tarjouksen ja
							tuet samalla kierrätystaloutta.
						</motion.p>
								...
								<AnimatePresence mode="wait">
									{currentStep === TOTAL_STEPS + 1 ? (
										...
										>
											...
											<h2 className="text-3xl-fluid font-bold text-[var(--color-neutral)] mb-4">
												Kiitos pyynnöstäsi!
											</h2>
											<p className="text-[var(--color-neutral)]/80 mb-8 max-w-md mx-auto">
												Olemme vastaanottaneet tietosi ja otamme sinuun yhteyttä
												sähköpostitse 24 tunnin sisällä.
											</p>
											<Button
												onClick={() => {
													setCurrentStep(1);
											...
											<div className="relative min-h-[220px] sm:min-h-[280px] md:min-h-[420px]">
														...
														<div className="bg-[var(--color-surface-3)]/50 p-4 sm:p-6 rounded-xl border border-[var(--color-border)]">
															...
															<div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
																<div className="space-y-2">
																	<Label className="text-[var(--color-neutral)]">
																		Koko
																	</Label>
																	<Combobox
																		options={RAM_OPTIONS}
															...
															<div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
																<div className="space-y-2">
																	...
																	<Combobox
																		...
																		onValueChange={(v) =>
																			handleInputChange("storageType", v)
																		}
																		placeholder="Valitse..."
																	/>
																	{errors.storageType && (
																...
																<div className="space-y-2">
																	...
																	<Combobox
																		...
																		onValueChange={(v) =>
																			handleInputChange("storageSize", v)
																		}
																		placeholder="Valitse..."
																	/>
																	{errors.storageSize && (
												...
												{currentStep === 3 && (
													...
													>
															...
															<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
																<div className="space-y-2">
																	...
																	<Combobox
																		...
																		onValueChange={(v) =>
																			handleInputChange("psuWattage", v)
																		}
																		placeholder="Valitse..."
																	/>
																	{errors.psuWattage && (
																...
																<div className="space-y-2">
																	...
																	<Combobox
																		...
																		onValueChange={(v) =>
																			handleInputChange("psuEfficiency", v)
																		}
																		placeholder="Valitse..."
																	/>
														...
														<div className="space-y-4">
															<Label className="text-[var(--color-neutral)] font-semibold">
																Otsikko
															</Label>
															<Input
														...
														<div className="space-y-4">
															...
															<Input
																...
																onChange={(e) =>
																	handleInputChange("caseModel", e.target.value)
																}
																className="bg-[var(--color-surface-3)] border-[var(--color-border)] text-[var(--color-neutral)] placeholder-[var(--color-neutral)]/50 focus:border-[var(--color-primary)] focus:ring-[var(--color-primary)]/40"
															/>
														</div>
														...
														<div className="space-y-4">
															...
															<Combobox
																...
																onValueChange={(v) =>
																	handleInputChange("motherboardQuality", v)
																}
																placeholder="Valitse emolevyn laatu..."
															/>
															<p className="text-xs text-[var(--color-text-tertiary)]">
																Valitsemme varovaisen oletuksen, jos et ole
																varma. Laadukas kustomoitu emolevy voi nostaa
																hyvitystä hieman.
															</p>
														</div>
											...
											<div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-6 border-t border-[var(--color-border)]/50">
												...
												{currentStep < TOTAL_STEPS ? (
													...
													<Button
														type="submit"
														disabled={
															createTradeInSubmissionMutation.status ===
																"pending" ||
															!formData.cpu ||
															!formData.gpu ||
															!formData.title ||
													...
													>
														...
														"pending" ? (
															<>
																<div
																	className="w-4 h-4 border-2 border-[var(--color-neutral)] border-t-transparent rounded-full animate-spin"
																	aria-hidden
																/>
					...
					<div className="xl:col-span-1 space-y-lg">
								...
								{[
									{
										...
										title: "Täytä tiedot",
										description:
											"Saat heti alustavan hinta-arvion komponenttiesi perusteella.",
									},
									{
										icon: Zap,
										title: "Lähetä pyyntö",
										description:
											"Otamme yhteyttä 24 tunnin sisällä sähköpostitse.",
									},
									{
										icon: Shield,
										title: "Tarkastus & Tarjous",
										description:
											"Tarkastamme koneen ja vahvistamme lopullisen hinnan.",
									},
									{
										icon: CheckCircle2,
										title: "Maksu",
										description:
											"Hyväksyttyäsi tarjouksen, saat rahat nopeasti tilillesi.",
									},
								].map((step, i) => (
									<div key={i} className="flex items-start space-x-3">
										...
										<div>
											...
											<p className="text-[var(--color-neutral)]/80 text-sm-fluid mt-1">
												{step.description}
											</p>
										</div>
									</div>
						...
						<Card className="bg-gradient-to-br mt-4 from-[var(--color-surface-3)]/50 to-[var(--color-surface-4)]/50 border-[var(--color-border)]/50 shadow-lg">
							<CardContent className="p-lg">
								<h3 className="font-bold text-[var(--color-neutral)] mb-4">
									Miksi valita Repur.fi?
								</h3>
								<div className="space-y-3">
									{[
										{
											icon: Shield,
											text: "12 kuukauden takuu kaikille koneille",
										},
										...
										<div key={i} className="flex items-center space-x-2">
											...
											<span className="text-sm-fluid text-[var(--color-neutral)]/80">
												{benefit.text}
											</span>
										</div>
									))}
