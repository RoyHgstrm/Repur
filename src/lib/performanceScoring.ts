...
const GPU_SCORES: Readonly<Record<string, number>> = {
  // NVIDIA 40 Series
  rtx4090: 100, rtx4080super: 98, rtx4080: 95, rtx4070tisuper: 92,
  rtx4070ti: 90, rtx4070super: 88, rtx4070: 85, rtx4060ti: 78,
  rtx4060: 75, rtx4050: 68,

  // NVIDIA 30 Series
  rtx3090ti: 96, rtx3090: 94, rtx3080ti: 92, rtx3080: 89,
  rtx3070ti: 85, rtx3070: 82, rtx3060ti: 80, rtx3060: 76,

  // AMD 7000 Series
  rx7900xtx: 99, rx7900xt: 94, rx7800xt: 88, rx7700xt: 83,
  rx7600: 76, rx7600s: 74,

  // Intel ARC
  a770: 79, a750: 74, a580: 68, a380: 58,

  // Mobile GPUs (keep only entries that don't duplicate desktop keys)
  rtx4090mobile: 92, rtx4080mobile: 87, rtx4070mobile: 82,
  rx7900m: 86, arc370m: 62,
};
 ...
 */
const GPU_SERIES_TO_ARCH: Readonly<Record<string, keyof typeof GPU_ARCHITECTURES>> = {
  'nvidia:40': 'ada-lovelace',
  'nvidia:30': 'ampere',
...
const CPU_SCORES: Readonly<Record<string, number>> = {
  // Intel 14th Gen
  '14900k': 100, '14900': 99, '14700k': 96, '14700': 95,
  '14600k': 90, '14600': 89, '14500': 85, '14400': 82,

  // AMD 7000 Series
  '7950x3d': 100, '7950x': 99, '7900x3d': 97, '7900x': 96,
  '7800x3d': 98, '7700x': 92, '7600x': 88, '7500f': 85,

  // Mobile CPUs
  '7945hx': 97, '7845hx': 92, '7745hx': 88,
  '13980hx': 96, '13900h': 90, '13700h': 87,
};
 ...
 * `gen`: Sukupolven indeksi (korkeampi = uudempi)
 */
const CPU_ARCHITECTURES: Readonly<Record<string, { baseScore: number; gen: number }>> = {
 ...
 */
const CPU_SERIES_TO_ARCH: Readonly<Record<string, keyof typeof CPU_ARCHITECTURES>> = {
...
const RAM_SCORES = {
   ...
   */
  capacity: { 4: 20, 8: 40, 16: 70, 32: 90, 64: 100, 128: 100 } as Readonly<Record<number, number>>,
  /**
   * @comment
   * RAM type scores.
   */
  type: { ddr3: 0, ddr4: 20, ddr5: 40 } as Readonly<Record<'ddr3' | 'ddr4' | 'ddr5', number>>,
  /**
   * @comment
  ...
  speed: {
    ...
    ddr4: { 2133: 5, 2400: 10, 2666: 15, 3000: 20, 3200: 25, 3600: 30, 4000: 35 },
    ddr5: { 4800: 20, 5200: 25, 5600: 30, 6000: 35, 6400: 40, 7200: 45, 8000: 50 },
  } as Readonly<Record<'ddr3' | 'ddr4' | 'ddr5', Readonly<Record<number, number>>>>,
};
...
const STORAGE_SCORES = {
   ...
   * Storage type scores.
   */
  type: { hdd: 30, sata: 60, nvme: 90, pcie4: 100, pcie5: 100 } as Readonly<Record<'hdd' | 'sata' | 'nvme' | 'pcie4' | 'pcie5', number>>,
  /**
   * @comment
   * Storage capacity scores in GB.
   */
  capacity: { 256: 20, 512: 40, 1000: 70, 2000: 90, 4000: 100 } as Readonly<Record<number, number>>,
};
 ...
 */
const normalizeComponent = (component: string | null, type: 'gpu' | 'cpu' | 'ram' | 'storage'): string => {
  if (!component) return '';
  let normalized = component
    .toLowerCase()
    .replace(/[^a-z0-9]/g, ''); // Poista erikoismerkit

  // Yleiset suodattimet
  normalized = normalized.replace(/(ti|super|xtx?|max[qp]|mobile|m|oc|boost|edition|processor|cpu|gpu|ram|memory|gb|tb|tb)/g, '');
  normalized = normalized.replace(/(intel|amd|nvidia|core|ryzen|radeon|ge?force|v[ge]a|uhd|iris|arc)/g, '');

  if (type === 'ram') {
    // Säilytä DDR-tyyppi (esim. ddr4) ja nopeus (esim. 3200)
    normalized = normalized.replace(/(ddr[345]?)/g, (match) => match);
    normalized = normalized.replace(/(\d{3,4})\s*(mhz|mt\/s)/g, (match, p1) => p1);
  } else if (type === 'storage') {
    // Muunna TB GB:ksi
    normalized = normalized.replace(/(\d+)\s*(gb|tb)/g, (match, p1, p2) => (p2 === 'tb' ? `${parseInt(p1, 10) * 1000}` : p1));
  }
...
const getRamScore = (ram: string): number => {
  ...
  const type: 'ddr3' | 'ddr4' | 'ddr5' = typeMatch ? (typeMatch[1] as 'ddr3' | 'ddr4' | 'ddr5') : 'ddr4'; // Oletus DDR4, jos ei löydy
...
export const computePerformanceScore = (specs: PerformanceSpecs): number => {
  ...
  if (normalizedGpu) {
    ...
    console.info(`[perf-score] Tuntematon näytönohjain: raaka="${specs.gpu}" normalisoitu="${normalizedGpu}"`);
    }
  }
  ...
  if (normalizedCpu) {
    ...
    console.info(`[perf-score] Tuntematon prosessori: raaka="${specs.cpu}" normalisoitu="${normalizedCpu}"`);
    }
  }
...
export const getPerformanceTier = (score: number): PerformanceTier => {
  if (score >= 95)
    return { level: 'Elite', color: '#8b5cf6', description: 'Äärimmäinen peli-/työasemasuorituskyky' };
  if (score >= 85)
    return { level: 'Pro', color: '#ef4444', description: 'Huippuluokan pelaamista ja tuottavuutta' };
  if (score >= 70)
    return { level: 'Performance', color: '#3b82f6', description: 'Erinomainen 1440p-pelaamiskokemus' };
  if (score >= 55)
    return { level: 'Mainstream', color: '#10b981', description: 'Vankka 1080p-pelaamiskokemus' };
  return { level: 'Entry', color: '#6b7280', description: 'Satunnaiseen pelaamiseen ja tuottavuuteen' };
};
