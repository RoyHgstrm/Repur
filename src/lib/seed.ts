...
async function main() {
	...
	if (!testUser) {
		...
		[testUser] = await db
			...
			.returning();
		console.log(
			`Created new test user: ${testUser?.email} (ID: ${testUser?.id}, ClerkID: ${testUser?.clerkId})`,
		);
	} else {
		...
		if (testUser.id !== internalUserId || testUser.clerkId !== clerkUserId) {
			[testUser] = await db
				...
				.returning();
			console.log(
				`Updated existing test user: ${testUser?.email} (ID: ${testUser?.id}, ClerkID: ${testUser?.clerkId})`,
			);
		}
		console.log(
			`Found existing test user: ${testUser.email} (ID: ${testUser.id}, ClerkID: ${testUser.clerkId})`,
		);
	}
	...
	if (!testListing) {
		...
		[testListing] = await db
			...
			.returning();
		console.log(
			`Created new test listing: ${testListing?.title} (ID: ${testListing?.id}, SellerID: ${testListing?.sellerId}, Status: ${testListing?.status})`,
		);
	} else {
		...
		if (testListing.status !== listingStatusEnum.enumValues[1]) {
			[testListing] = await db
				...
				.returning();
			console.log(
				`Updated existing test listing: ${testListing?.title} (ID: ${testListing?.id}, Status: ${testListing?.status})`,
			);
		}
		console.log(
			`Found existing test listing: ${testListing.title} (ID: ${testListing.id}, Status: ${testListing.status})`,
		);
	}
