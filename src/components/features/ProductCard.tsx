...
export const ProductCard = ({
	...
	const discountAmountNum =
		"discountAmount" in listing && (listing as any).discountAmount != null
	...
	const hasWindow =
		...
		(listing as any).discountEnd
			? now >= new Date(String((listing as any).discountStart)) &&
					...
					<Card className="h-full flex flex-col sm:flex-row items-stretch bg-surface-2 border-[var(--color-border-light)] hover:border-[var(--color-primary)]/40 shadow-sm hover:shadow-md transition-transform duration-200 ease-out transform-gpu overflow-hidden">
						...
						<div className="flex-1 min-w-0 p-4 sm:p-5 flex flex-col">
							...
							<div className="flex flex-col sm:flex-row items-start justify-between gap-2 mb-3">
								...
								<Badge
									className={cn(
										"text-[10px] sm:text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 mt-1 sm:mt-0",
							...
							<div className="space-y-2 mb-4">
								<div className="flex items-center gap-2 p-2 bg-surface-1 rounded-lg transition-colors hover:bg-surface-1/80">
									...
									<span className="text-xs text-secondary font-medium min-w-0">
										CPU:
									</span>
									<span className="text-xs text-primary font-semibold truncate flex-1">
										{listing.cpu ?? "Ei tietoa"}
									</span>
								</div>
								<div className="flex items-center gap-2 p-2 bg-surface-1 rounded-lg transition-colors hover:bg-surface-1/80">
								...
								<div className="flex items-baseline gap-2">
									{hasWindow && discountAmountNum > 0 ? (
										<>
											...
											<span className="text-xl font-extrabold bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-accent)] bg-clip-text text-transparent">
												{finalPrice} €
											</span>
										</>
									) : (
				...
				<Card className="h-full flex flex-col bg-gradient-to-br from-surface-2 to-surface-3 border-[var(--color-border-light)] hover:border-[var(--color-primary)]/50 shadow-sm hover:shadow-md transition-transform duration-200 ease-out group-hover:-translate-y-1 sm:group-hover:-translate-y-2 overflow-hidden">
					{hasWindow && discountAmountNum > 0 && (
						<div className="absolute top-3 right-3 z-10">
							<span
								className="discount-badge badge-spotlight"
								aria-label="Alennus voimassa"
							>
								✨ ALENNUS
							</span>
						</div>
              ...
              "
					...
					<CardContent className="flex-grow space-y-3 text-sm px-3 sm:px-4 md:px-6">
						...
						<div className="flex items-center gap-3">
							<div className="min-w-[100px] text-xs text-tertiary">
								Suorituskyky
							</div>
							<Progress value={perfScore} className="h-2 flex-1" />
							<div className="text-xs font-semibold text-primary w-10 text-right">
						...
						<div className="flex gap-2">
							...
							<Button
								...
								onClick={(e) => {
									...
									if (navigator.share) {
										navigator.share({
											url: `${window.location.origin}/osta/${listing.id}`,
											title: listing.title,
										});
									} else {
										navigator.clipboard.writeText(
											`${window.location.origin}/osta/${listing.id}`,
										);
									}
								}}
								title="Jaa"
