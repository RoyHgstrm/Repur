...
const SidebarLink = ({ href, icon, children, isActive }: SidebarLinkProps) => (
  <Link
    ...
    className={cn(
      ...
      isActive ? "bg-[var(--color-surface-3)] text-[var(--color-primary)]" : "text-[var(--color-neutral)]/80"
    )}
  >
    {icon}
    {children}
  ...
  return (
    <aside className="sticky top-24 h-[calc(100vh-theme(spacing.24))] overflow-y-auto bg-gradient-to-br from-surface-2 to-surface-3 border-[var(--color-border-light)] shadow-xl rounded-xl p-6">
      <nav className="space-y-4">
        <SidebarLink href="/osta" icon={<Home className="h-5 w-5" />}>Osta Tietokone</SidebarLink>
        <SidebarLink href="/myy" icon={<Package className="h-5 w-5" />}>Myy Koneesi</SidebarLink>
        <SidebarLink href="/tuki" icon={<LifeBuoy className="h-5 w-5" />}>Asiakastuki</SidebarLink>
      </nav>
    </aside>
  );
}